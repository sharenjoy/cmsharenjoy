...
class Parser {
     ...
     */
	public function treeBuilder($items, $html, $tag = 'ul')
    {
        if (empty($items)) {
            return;
        }
        ...
        foreach ($items as $item) {
            ...
            if ($item->children and !$item->children->isEmpty()) {
                ...
                $item_array['children'] = '<'.$tag.'>' . $this->treeBuilder($item->children, $html) . '</'.$tag.'>';

            } elseif (count($item['children'])) {
                $item_array['children'] = '<'.$tag.'>' . $this->treeBuilder($item['children'], $html) . '</'.$tag.'>';
            } else {
            	$item_array['children'] = null;
            }

            // now that the children html is sorted we parse the html that they passed
            $output .= $this->parse_string($html, $item_array, true);
        }
