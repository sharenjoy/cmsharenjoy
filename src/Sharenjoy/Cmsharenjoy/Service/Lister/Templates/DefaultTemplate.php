...
class DefaultTemplate extends TemplateAbstract implements TemplateInterface {
    ...
    protected function pagination()
    {
        ...
        $content .= '&nbsp;&nbsp;'.pick_trans('pagination_desc', ['from'=>$this->data['items']->getFrom(), 'to'=>$this->data['items']->getTo(), 'total'=>$this->data['items']->getTotal()]);
        $content .= Form::close();
        $content .= '</div></div><div class="'.$this->data['config']['pagination-div-class'].'"><div class="dataTables_paginate paging_bootstrap">'.$this->data['items']->links().'</div></div></div>';
