...
class FilerHandler implements FilerInterface {
    ...
    {
        $this->path        = public_path().Config::get('cmsharenjoy::filer.path');
        $this->thumbPath   = public_path().Config::get('cmsharenjoy::filer.thumbPath');
        $this->_cache_path = public_path().Config::get('cmsharenjoy::filer.cache_path');
    ...
    **/
    public function createContainer($container, $location, $id = 0)
    {
        ...
        if ($result) {
            return $this->result(true, trans('cmsharenjoy::files.container_created'), $container);
        }

        return $this->result(false, trans('cmsharenjoy::files.error_container'), $container);
    }
    ...
    **/
    public function checkContainer($name, $location)
    {
        ...
        foreach ($containers as $container) {
            if ($name === $container) {
                return $this->result(true, trans('cmsharenjoy::files.container_exists'), $name);
            }
        }
        return $this->result(false, trans('cmsharenjoy::files.container_not_exists'), $name);
    }
    ...
    **/
    public function renameFolder($id = 0, $name)
    {
        ...
        return $this->result(true, trans('cmsharenjoy::files.item_updated'), $folder->name, $folder->toArray());
    }
    ...
    public function upload($folder_id, $name = false, $field = 'userfile', $width = false, $height = false, $ratio = false, $allowed_types = false, $alt = NULL, $replace_file = false)
    {
        ...
        if ($folder) {
            ...
            if ($field->move($this->path, $full_file_name)) {
                ...
                return $this->result(true, trans('cmsharenjoy::files.file_uploaded'), $data['name'], array('id' => $file_id) + $data);
            }
            else 
            {
                header("Connection: close");
                return $this->result(false, trans('cmsharenjoy::files.upload_error'));
            }
        }
        else
        {
            header("Connection: close");
            return $this->result(false, trans('cmsharenjoy::files.specify_valid_folder'));
        }
    }
    ...
    **/
    public function move($file_id, $new_name = false, $location = false, $new_location = 'local', $container = '')
    {
        ...
        if (! $file) {
            return $this->result(false, trans('cmsharenjoy::files.item_not_found'), $new_name ? $new_name : $file_id);
        }
        ...
        if ($file->folder->location === 'local' and $new_location === 'local') {
            ...
            if (file_exists($this->path.$file->filename)) {
                ...
                return $this->result(true, trans('cmsharenjoy::files.item_updated'), $new_name, $data);
            } else {
                return $this->result(false, trans('cmsharenjoy::files.item_not_found'), $file->name);
            }
            ...
            if (in_array($container, $containers)) {
                ...
                return $this->result(true, trans('cmsharenjoy::files.file_uploaded'), $new_name, $extra_data + $data);
            }

            return $this->result(false, trans('cmsharenjoy::files.invalid_container'), $container);
            ...
            if ($curl_result) {
                ...
                return $this->result(true, trans('cmsharenjoy::files.file_moved'), $file->name, $data);
            } else {
                return $this->result(false, trans('cmsharenjoy::files.unsuccessful_fetch'), $file);
            }
            ...
            return $this->result(true, trans('cmsharenjoy::files.file_moved'), $file->name, $extra_data + $data);
        }
    }
    ...
    **/
    public function getFile($identifier = 0)
    {
        ...
        $message = $results ? null : trans('cmsharenjoy::files.no_records_found');
    ...
    **/
    public function listFiles($location = 'local', $container = '')
    {
                    ...
                    if ($location === 'amazon-s3') {
                        ...
                        if (! $cf_container['cdn_enabled']) {
                            ...
                            if ( ! $path = ci()->storage->create_container($container, 'public')) {
                                ...
                                return $this->result(false, trans('cmsharenjoy::files.enable_cdn'), $container);
                            }
                        }
        ...
        $message = $files ? null : trans('cmsharenjoy::files.no_records_found');
    ...
    **/
    public function synchronize($folder_id)
    {
        ...
        if ($files['status']) {
            ...
            return $this->result(true, trans('cmsharenjoy::files.synchronization_complete'), $folder->name, $files['data']);
        } else {
            return $this->result(null, trans('cmsharenjoy::files.no_records_found'));
        }
    }
    ...
    **/
    public function renameFile($id = 0, $name)
    {
        ...
        return $this->result(true, trans('cmsharenjoy::files.item_updated'), $name, array('id' => $id, 'name' => $name));
    }
    ...
    **/
    public function replaceFile($to_replace, $folder_id, $name = false, $field = 'userfile', $width = false, $height = false, $ratio = false, $allowed_types = false, $alt_attribute = null)
    {
        ...
        return $this->result(false, trans('cmsharenjoy::files.item_not_found'), $id);
    }
    ...
    public function deleteFile($id = 0)
    {
        if ($file = File::find($id))
        {
            if ($this->_unlinkFile($file))
            {
                ...
                return $this->result(true, trans('cmsharenjoy::files.item_deleted'), $file->name);
            }
        }
    ...
    **/
    public function search($search, $limit = 5)
    {
        ...
        return $this->result(false, trans('cmsharenjoy::files.no_records_found'));
    }
    ...
    **/
    public function checkDir($path)
    {
        ...
        elseif ( ! is_dir($path))
        {
            ...
            {
                return $this->result(false, trans('cmsharenjoy::files.mkdir_error'), $path);
            }
            else
            {
        ...
        {
            return $this->result(false, trans('cmsharenjoy::files.chmod_error'));
        }
    }
    ...
    **/
    private function _checkExt($field)
    {
        if ( ! empty($field)) {
            ...
            $allowed = Config::get('cmsharenjoy::filer.allowed_file_ext');
            ...
            if ( ! $this->_ext) {
                return $this->result(false, trans('cmsharenjoy::files.invalid_extension'), $field->getClientOriginalName());
            }
        } else {
            return $this->result(false, trans('cmsharenjoy::files.upload_error'));
        }
    ...
    **/
    private function _getFileInfo($filename)
    {
        ...
        $allowed	= Config::get('cmsharenjoy::files.allowed_file_ext');
